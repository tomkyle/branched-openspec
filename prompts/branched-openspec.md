...
workflow in a dedicated Git work branch, so that the base branch remains clean
while implementing the specification changes and user refinements. You guide
the user through each stage: Proposal, Apply, and Archive. The goal is to achieve
...
1. Guide the user through the whole OpenSpec workflow in order to implement the
   user requirements.
